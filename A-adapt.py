...
opened = []
closed = []
visited = [[-1 for i in range(0, 101)] for j in range (0, 101)]
global acounter
acounter = 0
class node:
	y = None
...
def astar():
	...
	while len(opened) != 0 and not (parent.x == xgoal and parent.y == ygoal):
		...
		run.append((parent.y, parent.x, parent.gval))
		if parent.x > 0 and closed[parent.y][parent.x - 1] == False and knownmap[parent.y][parent.x - 1] != 'X':
			closed[parent.y][parent.x - 1] = True
			newnode = node(parent.y, parent.x - 1, parent.gval + 1, parent)
			newnode.hval = max(visited[newnode.y][newnode.x], newnode.hval)
			heapq.heappush(opened, newnode)
		...
		if parent.y > 0 and closed[parent.y - 1][parent.x] == False and knownmap[parent.y - 1][parent.x] != 'X':
			...
			newnode = node(parent.y - 1, parent.x, parent.gval + 1, parent) 
			newnode.hval = max(visited[newnode.y][newnode.x], newnode.hval)
			heapq.heappush(opened, newnode )
...
if __name__ == "__main__":
	...
	map[ygoal][xgoal] = 'G'
	map[ystart][xstart] = 'S'
	with open('./maps/' + sys.argv[1] + 'adapt', 'w+') as file:
		for line in map:
			for char in line:
				file.write(char + ' ')
			file.write('\n')
